...
const ProductPage = ({type}) => {
    ...
    const products = items.map((item) => <div key={item.id}><ProductBlock text={item.text} info={item.info} related={item.related} type={type} path={`/${camelize(type.name)}/${item.id}`} {...item} /></div>);
