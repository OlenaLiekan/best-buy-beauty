...
const BrandsBlock = () => {
  ...
  const [numberOfSlides, setNumberOfSlides] = React.useState(3);
  ...
  React.useEffect(() => {
    if (brands.length > 0) {
      const result = Math.ceil(brands.length / 3);
      setNumberOfSlides(result);
    }
  }, [brands.length]);

  const slidesArray = [...new Array(numberOfSlides)];
                        ...
                        <SwiperSlide key={index}>                 
                          <div className="block-companies__slide slide-companies-block">
                            <div className="slide-companies-block__content">
                              {brands.slice(index * 3, (index + 1) * 3).map((brand) => 
