...
const AdminInitial = () => {
    ...
    const [editTickerMode, setEditTickerMode] = React.useState(false);
    const {
        serverDomain,
        imagesCloud,
        isPromoPage,
    ...
    React.useEffect(() => {
        ...
        axios.get(`${serverDomain}api/ticker/2`)
            .then((res) => {
                setTicker(res.data);
                setTickerText(res.data.text);
                setTickerLoading(false);
            });
    }, [serverDomain, editTickerMode]);
    ...
    const onChangeTickerText = (event) => {
    ...
    const success = () => {
        ...
        if (editTickerMode) {
            window.alert('Ticker atualizado com sucesso!');
            setEditTickerMode(false); 
        }
    };
    ...
    const updateTickerText = (e) => {
        e.preventDefault();
        const tickerId = 2;
        const formData = new FormData();
        formData.set('text', tickerText);
        updateTicker(formData, tickerId).then(data => success()).catch((err) => message());
    };
    ...
    return (
        <>
            ...
            }
            {
                ...
                !editTickerMode
                    ?
                    <div className={styles.adminInitialTicker}>
                        <div>Envio express gratuito a partir de 89.00 €</div>
                        <div onClick={() => setEditTickerMode(true)}>
                    ...
                    !editLogoMode
                    &&
                    <form onSubmit={updateTickerText} className={styles.tickerForm}>
                        <textarea
                            value={tickerText}
                            onChange={onChangeTickerText}
                            className={styles.tickerTextarea}
                            placeholder='Digite o novo texto'
                        />
                        <button type='submit' className={styles.upBtn}>
                            Update ticker
                        </button>
                        <button onClick={() => setEditTickerMode(false)} type='button' className={styles.cancelBtn}>
                            Cancelar
                        </button>
                    </form>    
            }
